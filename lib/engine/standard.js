    ...
    init(done) {
        ...
        if (postDataPluginIdx && postDataPluginIdx != '') {
            ...
            this.postDataAccess.Init(parameters, (err) => {
                if (!err) {
                    var commentDataPluginIdx = this.dataAccess.GetActivePlugin(plugin_1.PluginType.CommentsDataAccess);
                    if (commentDataPluginIdx && commentDataPluginIdx != '') {
                        var currentCommentPlugin = this.pluginManager.GetPlugin(commentDataPluginIdx);
                        this.commentDataAccess = (new currentCommentPlugin.Class['Plugin']());
                        var parameters = this.dataAccess.GetPluginParameters(plugin_1.PluginType.CommentsDataAccess);
                        parameters['getuser'] = this.dataAccess.GetUser;
                        ...
                        this.commentDataAccess.Init(parameters, (err) => {
                            if (!err) {
                                ...
                                this.logError(errors_1.Errors.UnableToStartcommentDataAccess.Text);
                            }
                        });
                    }
                }
                else {
                    this.logError(errors_1.Errors.UnableToStartPostDataAccess.Text);
                }
            });
        }
    }
